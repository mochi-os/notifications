...
def database_create():
	mochi.db.query("""create table notifications (
...
def database_upgrade(version):
	if version == 2:
		mochi.db.query("drop table notifications")
		mochi.db.query("""create table notifications (
			id text not null primary key,
			app text not null,
			category text not null,
...
def function_expire():
	now = mochi.time.now()
	mochi.db.query("delete from notifications where read = 0 and created < ?", now - 30 * 86400)
	mochi.db.query("delete from notifications where read != 0 and created < ?", now - 7 * 86400)
...
def function_create(app, category, object, content, link):
	...
	if existing and existing["read"] == 0:
		...
		mochi.db.query("update notifications set content = ?, count = ?, created = ? where id = ?", content, count, now, existing["id"])
		id = existing["id"]
	else:
		id = mochi.uid()
		mochi.db.query("""replace into notifications (id, app, category, object, content, link, count, created, read)
...
def function_list():
	return mochi.db.query("select * from notifications order by created desc")

def function_read(id):
	now = mochi.time.now()
	mochi.db.query("update notifications set read = ? where id = ?", now, id)
	mochi.websocket.write("notifications", {"type": "read", "id": id})

def function_read_all():
	now = mochi.time.now()
	mochi.db.query("update notifications set read = ?", now)
